...
function page() {
    ...
    return (
        <React.Fragment>
            ...
            />
            <section className='md:w-[70vw] mx-auto  my-4'>
                <div className='py-8 '>
                    <h3 className="text-center text-4xl pb-4">OUR SERVICES INCLUDE</h3>
                    <p className='text-center'>At Vertical Click’s USA, our team of experienced marketers and designers can help you create effective email <br /> marketing campaigns that resonate with your audience.</p>

                    <div className='grid grid-cols-3 gap-4 mx-auto auto-rows-fr'>
                        {analyticsItems.map((item, index) => (
                            <div
                                key={index}
                                className={`pt-4 px-4 relative mt-4 bg-[#f2ebff] gap-2 flex flex-col justify-between rounded-md 
            ${index === 0 || index === analyticsItems.length - 1 ? 'col-span-2' : 'col-span-1'}`}
                            >
                                <div>
                                ...
                                <div className='flex justify-around items-end gap-6'>
                                    <div>
                                        <img src={item.img} width={100} alt="" className='md:w-100' />
                                    </div>
                                </div>
                            </div>
                        ))}
                        ...
                        analyticBottomContent.map((item  , index )=>{
                              return(
                                  <div className='flex justify-between items-start py-4'>
                                      <div className='flex-1 text-4xl -translate-y-2'>{item.title}</div>
